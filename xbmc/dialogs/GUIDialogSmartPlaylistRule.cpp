...
void CGUIDialogSmartPlaylistRule::OnBrowse()
{
  ...
  else if ((m_rule.m_field == static_cast<int>(Field::ALBUM_CODEC)) ||
           (m_rule.m_field == static_cast<int>(Field::BITS_PER_SAMPLE)))
  {
    std::string reqField;
    if (m_rule.m_field == static_cast<int>(Field::ALBUM_CODEC))
    {
      iLabel = 21446;
      reqField = "strCodec";
    }
    else if (m_rule.m_field == static_cast<int>(Field::BITS_PER_SAMPLE))
    {
      iLabel = 612;
