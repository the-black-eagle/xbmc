    ...
    if (m_profileManager.GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      ...
      for (const auto &i : mapActors)
      {
        ...
        if (idContent == VIDEODB_CONTENT_MUSICVIDEOS)
        {
          ...
          if (db.Open())
          {
            int idArtist = db.GetArtistByName(i.second.name);
            if (idArtist > 0)
            {
              db.GetArtist(idArtist, artist);
              pItem->GetVideoInfoTag()->m_strPlot = artist.strBiography;
              pItem->GetVideoInfoTag()->m_strPictureURL.ParseFromData(
                  artist.thumbURL.GetFirstThumbUrl());
            }
            db.Close();
          }
        }
      ...
      while (!m_pDS->eof())
      {
        try
        {
          ...
          if (idContent == VIDEODB_CONTENT_MUSICVIDEOS)
          {
            ...
            if(db.Open())
            {
              int idArtist = db.GetArtistByName(m_pDS->fv(1).get_asString());
              if (idArtist > 0)
              {
                db.GetArtist(idArtist, artist);
                pItem->GetVideoInfoTag()->m_strPlot = artist.strBiography;
