...
bool CMusicDatabase::RemoveBoxset(int albumId)
{
  std::string strSQL;
  bool off = false;
  try
  {
    strSQL=PrepareSQL("UPDATE album SET bBoxedSet = %i WHERE idAlbum = %i", off, albumId);
  ...
  {
    CLog::Log(LOGERROR, "musicdatabase:unable to remove boxset (%s)", strSQL.c_str());
  }
  return false;
}
